...
class Scene2Scene_2_Topic__NonCooperation_Movement(Scene):
    def construct(self):
        ...
        def create_gandhi():
            body = Line(start=[0, -1, 0], end=[0, 0.5, 0], stroke_width=8, color=WHITE)
            head = Circle(radius=0.3, color=WHITE, fill_opacity=0.8).shift(UP*0.8)
            glasses = VGroup(
                Line(start=[-0.3, 0.7, 0], end=[-0.1, 0.7, 0]),
                Line(start=[0.1, 0.7, 0], end=[0.3, 0.7, 0]),
                ArcBetweenPoints([-0.3, 0.7, 0], [0.3, 0.7, 0], angle=-0.5)
            )
            spinning_wheel = VGroup(
            ...
            return VGroup(body, head, glasses, spinning_wheel)
        
        # Create protest symbols
        def create_protest_symbols():
            boycott = VGroup(
                Rectangle(width=1.5, height=1, color=RED),
                create_smart_text("BOYCOTT", font_size=24, color=RED)
            )
            swadeshi = VGroup(
                Star(n=5, outer_radius=0.7, color=GREEN),
                create_smart_text("SWADESHI", font_size=24, color=GREEN)
            )
            return VGroup(boycott, swadeshi).arrange(RIGHT, buff=1.5)
        
        # Create movement growth visualization
        def create_growth_visual():
            base = Line(LEFT*3, RIGHT*3, stroke_width=3)
            stem = Line(ORIGIN, UP*1.5, stroke_width=3)
            leaves = VGroup(
                Polygon([0,1.5,0], [-0.5,1,0], [0,0.5,0], color=GREEN),
                Polygon([0,1.5,0], [0.5,1,0], [0,0.5,0], color=GREEN)
            )
            roots = VGroup(*[
                Line(ORIGIN, np.array([np.cos(a), np.sin(a), 0])*0.7, stroke_width=2)
                for a in np.linspace(3*PI/4, 5*PI/4, 5)
            ])
            return VGroup(base, stem, leaves, roots)
        
        # Create timeline visualization
        def create_timeline():
            line = Line(LEFT*4, RIGHT*4, stroke_width=3)
            events = VGroup(
                self.create_timeline_event("1920", "Movement\nLaunched", UP),
                self.create_timeline_event("1922", "Chauri\nChaura", DOWN)
            )
            return VGroup(line, events)
        ...
        # Gandhi introduction
        gandhi = create_gandhi().move_to(get_zone_center("MAIN_CONTENT_AREA") + LEFT*3)
        self.play(DrawBorderThenFill(gandhi[1]), Create(gandhi[0]))
        self.play(Create(gandhi[2]), Create(gandhi[3]))
        self.play(Rotate(gandhi[3], angle=2*PI, run_time=2))
        self.wait(1)
        
        # Protest symbols animation
        symbols = create_protest_symbols().move_to(get_zone_center("MAIN_CONTENT_AREA") + RIGHT*2)
        self.play(
            LaggedStart(
                Create(symbols[0][0]),
                Write(symbols[0][1]),
        ...
        self.play(Indicate(symbols[0]), Indicate(symbols[1]))
        self.wait(1)
        
        # Movement growth animation
        plant = create_growth_visual().move_to(get_zone_center("MAIN_CONTENT_AREA") + DOWN*1)
        self.play(Create(plant[0]))
        self.play(GrowFromPoint(plant[1], plant[0].get_center()))
        self.play(DrawBorderThenFill(plant[2]), Create(plant[3]))
        self.play(plant.animate.scale(1.2), run_time=1.5)
        self.wait(1)
        
        # Timeline animation
        timeline = create_timeline().shift(DOWN*1.5)
        self.play(Create(timeline[0]))
        self.play(
            AnimationGroup(
                FadeIn(timeline[1][0], shift=UP),
                FadeIn(timeline[1][1], shift=DOWN),
                lag_ratio=0.5
            )
        )
        self.play(Circumscribe(timeline[1], color=YELLOW, run_time=2))
        self.wait(2)
    ...
    def create_timeline_event(self, year, text, direction):
        dot = Dot(radius=0.15, color=RED)
        year_text = create_smart_text(year, font_size=24)
        event_text = create_smart_text(text, font_size=20)
        group = VGroup(dot, year_text, event_text).arrange(direction, buff=0.2)
        return group
